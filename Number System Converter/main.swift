    ...
    for character in value.reversed() {
        if base == 16.0 {
            ...
            if let digit = Double(String(character)) {
                decimalEquivalent += digit * pow(se, exponent)
            } else {
                // It can't be switched to a double, so its A-F
                switch character {
                case "A":
                    decimalEquivalent += 10 * pow(se, exponent)
                case "B":
                    decimalEquivalent += 11 * pow(se, exponent)
                case "C":
                    decimalEquivalent += 12 * pow(se, exponent)
                case "D":
                    decimalEquivalent += 13 * pow(se, exponent)
                case "E":
                    decimalEquivalent += 14 * pow(se, exponent)
                case "F":
                    decimalEquivalent += 15 * pow(se, exponent)
                default:
                    break
                }
            }
